...
func Code44_A(val byte) byte {
	return ((val >> 1) & 0x55) | 0xAA
}

func Code44_B(val byte) byte {
	return (val & 0x55) | 0xAA
}

func deserialise_track(src []byte, track_number byte, is_prodos bool) []byte {
	var track_position uint32 = 0
	var sector byte

	dest := make([]byte, 6646)
	// Write gap one, which contains 48 self-sync bytes
	for loop := 0; loop < 48; loop++ {
		dest[track_position] = 0xFF
		track_position++
	}
	...
	for sector = 0; sector < 16; sector++ {
		...
		dest[track_position] = 0xD5
		track_position++
		dest[track_position] = 0xAA
		track_position++
		dest[track_position] = 0x96
		track_position++

		dest[track_position] = Code44_A(0xFE)
		track_position++
		dest[track_position] = Code44_B(0xFE)
		track_position++
		dest[track_position] = Code44_A(track_number)
		track_position++
		dest[track_position] = Code44_B(track_number)
		track_position++
		dest[track_position] = Code44_A(sector)
		track_position++
		dest[track_position] = Code44_B(sector)
		track_position++
		dest[track_position] = Code44_A(0xFE ^ (track_number ^ sector))
		track_position++
		dest[track_position] = Code44_B(0xFE ^ (track_number ^ sector))
		track_position++
		dest[track_position] = 0xDE
		track_position++
		dest[track_position] = 0xAA
		track_position++
		dest[track_position] = 0xEB
		track_position++
		...
		encode6_2(contents, src[int(logical_sector)*256:])
		for _, data := range contents {
			dest[track_position] = data
			track_position++
		}
